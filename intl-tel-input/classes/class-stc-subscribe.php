...
class STC_Subscribe {
	...
	private function subscribe_user() {
		...
		do_action( 'stc_before_subscribe', $user_id );
                ...
                if (get_post_status($user_id) == 'update_approval') {
...
//                phpmailer takes care of the UTF-8 characters		$subject = '=?UTF-8?B?' . base64_encode( $title ) . '?=';
	...
	private function subscriber_exists() {
		...
		$result = $wpdb->get_row( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type = %s AND post_status='publish' ", $data['email'], 'stc' ) );
...
//                error_log("Triggered by");
		...
		foreach ( $posts as $p ) {
                ...
                // check for Classified Ads post. They must be prepared to work with STC plugin
		$args = ['post_type' => 'ad_post',
			'post_status' => 'publish',
			'numberposts' => -1
                        ];
		$posts = get_posts( $args );
		foreach ( $posts as $p ) {
                        //get the STC status
                        $stc_status = get_post_meta( $p->ID, '_stc_notifier_status', true );
                        // we need to add the status to trigger STC for the first time
                        if ( empty( $stc_status ) ) {
                                update_post_meta( $p->ID, '_stc_notifier_status', 'outbox' ); // updating post meta to initiate the send email process.
                                update_post_meta( $p->ID, '_stc_notifier_request', '' );
                        }
                }
			...
			foreach ( $subscribers as $subscriber ) {
                                ...
                                if (count($keywords)===0 || (!isset($this->settings['enable_keyword_search']) || (isset($this->settings['enable_keyword_search']) && $this->settings['enable_keyword_search'] <> "1"))) {
                                        ...
                                        if (count($subscriber->categories) == 0){
                                                ...
                                                foreach ( $subscriber->categories as $key=>$categories ) {
                                                        // add compare signs for in_array().
                                                        $cat_name = get_term($categories, "")->name;
                                                        ...
                                                        if ( in_array( $categories, $post_cat_compare ) ) {
                                                                ...
                                                                $reason_p1 = sprintf(esc_html__('Taxonomy: "%s" and ', 'subscribe-to-category'), $cat_name);
                                                                $reason = "";
                                                                // can we also find a matching keyword?
                                                                if ($this->FindKeywords($post, $keywords, $search_areas, $reason)) {
                                                                        $emails[ $i ]['subscriber_id'] = $subscriber->ID;
                                                                        $emails[ $i ]['mobile_phone'] = $subscriber->mobile_phone;
                                                                        $emails[ $i ]['mobile_phone_status'] = $subscriber->mobile_phone_status;
                                                                        ...
                                                                        $emails[ $i ]['email'] = $subscriber->post_title;
                                                                        $emails[ $i ]['post_id'] = $post->ID;
                                                                        $emails[ $i ]['reason'] = $reason_p1 . $reason;
                                                                        $emails[ $i ]['merged'] = false;
                                                                        $i++;
                                                                        break;
                                                                }
